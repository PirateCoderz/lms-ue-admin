...
import UploadPdf from 'src/components/meritlist/UploadPdf';
...
const TABLE_HEAD = [
  ...
  { id: 'file', label: 'Url', alignRight: false },
  { id: '' },
];
...
export default function MeritListPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = meritList.map((n) => n.name);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - meritList.length) : 0;

  const filteredUsers = applySortFilter(meritList, getComparator(order, orderBy), filterName);

  const isNotFound = !filteredUsers.length && !!filterName;

  const getAllMeritLists = async () => {
  ...
  const handleDelete = async (id) => {
    const res = await dispatch(deleteMeritListById(id));
    if (res.payload) {
      setRefetch(!refetch);
    }
  };
  ...
  return (
    <>
                  ...
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    return (
                      <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <a href={file} target="_blank" rel="noreferrer">{title}</a>
                        </TableCell>
      ...
      <CustomizedDialogs title="Add New Merit List" open={newTeacherOpen} setOpen={setNewTeacherOpen}>
        <UploadPdf refetch={refetch} setRefetch={setRefetch} setOpen={setNewTeacherOpen}/>
      </CustomizedDialogs>
