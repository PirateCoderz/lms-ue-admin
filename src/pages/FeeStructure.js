...
export default function FeeStructure() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = feeStructure.map((n) => n.name);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - feeStructure.length) : 0;

  const filteredUsers = applySortFilter(feeStructure, getComparator(order, orderBy), filterName);
  ...
  const handleDelete = async (id) => {
    const res = await dispatch(deleteFeeStructuretById(id));
    if (res.payload) {
      setRefetch(!refetch);
    }
  };
  ...
  return (
    <>
      ...
      <CustomizedDialogs title="Add New Fee Structure" open={newTeacherOpen} setOpen={setNewTeacherOpen}>
        <UploadFee refetch={refetch} setRefetch={setRefetch} setOpen={setNewTeacherOpen}/>
      </CustomizedDialogs>
